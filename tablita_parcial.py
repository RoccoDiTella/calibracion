...
def prepare_data_with_encoder(df, label_encoder):
    """Prepare data for calibration with pre-fitted encoder"""
    # Extract logits
    ...
    # Encode subjects using pre-fitted encoder
    topics = label_encoder.transform(df['subject'].values)
    
    print(f"Data preparation:")
    print(f"  Samples: {len(logits)}")
    print(f"  Unique subjects in this set: {df['subject'].nunique()}")
    print(f"  Logits shape: {logits.shape}")
    
    return logits, labels, topics
...
def run_mmlu_experiment():
    ...
    # Create unified label encoder for all subjects
    all_subjects = pd.concat([test_df['subject'], validation_df['subject']]).unique()
    label_encoder = LabelEncoder()
    label_encoder.fit(all_subjects)
    n_topics = len(all_subjects)
    
    print(f"Found {n_topics} unique subjects across both sets")
    
    # Prepare data with unified encoder
    # Use "test" set for training/CV (confusing MMLU naming)
    train_logits, train_labels, train_topics = prepare_data_with_encoder(test_df, label_encoder)
    # Use "validation" set for final testing  
    test_logits, test_labels, test_topics = prepare_data_with_encoder(validation_df, label_encoder)
